...
- `Profile` — user (OneToOne to User), role (`teacher`/`student`) — the only thing distinguishing a teacher account from a student account in the shared `auth.User` table; always set explicitly at signup/activation, never defaulted. `workspaces.decorators.teacher_required`/`student_required` gate every view on this.
- `RosterInvite` — workspace (FK), display_name, code (unique, teacher-facing activation code), created_at, claimed_at (null = still usable — the actual "used" flag, not `student`), student (FK to User, nullable, `on_delete=SET_NULL`) — a teacher-issued one-time code a named student redeems to create their own account (see "Student accounts" below)
...
The original anonymous join-by-code flow (`student_join`, `StudentSession` with no `student` FK, no password, no account) is unchanged and stays the default — this is additive, not a replacement, per the Philippines Data Privacy Act's data-minimization posture this app aims for (see `CLAUDE.md`'s minors constraint below). A student who wants their joined workspaces + chat history to persist across devices/logins can hold a real account instead:

- **Account creation is teacher-gated, not open self-signup.** A teacher adds a named student to a workspace's roster (`workspace_dashboard`'s "Student accounts" section → `roster_add`), which issues a one-time `RosterInvite.code`. The student redeems it at `/student/activate/` (`student_signup_activate`) to pick their own username/password (+ optional email — never required; no self-service password reset exists yet, since there's no email to reset via for students who skip it — teacher/admin-assisted reset is a known follow-up, not built).
...
- **Not built yet, flagged as follow-ups**: password reset (email- or teacher-assisted); a written consent notice for the activation step itself (a `docs/student_account_activation_notice.md`, following `docs/teacher_account_consent_notice.md`'s structure) — worth writing before this reaches real students.
